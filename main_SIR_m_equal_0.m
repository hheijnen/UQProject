...
%optimize only alpha and S0
opt_prior = optimize_prior_SIR_m_equal_0(data,prior);
...
parameters = {
    ...
    {'S0',      opt_prior(3),    0,   upperbounds(4)       }
};

%% Sampling

[result, chain, s2chain] = postSIR(data,parameters,burnsamples,runsamples,@SIRss_m_eq_0);
...
chain = [chain(:,1) chain(:,2) zeros(runsamples,1) chain(:,3)]; %add m to the params
for i=1:N
...
title('Flu-season 10/11, SIR, m=0')
%% Calculate Hessian
